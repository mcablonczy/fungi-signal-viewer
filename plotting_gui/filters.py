...
def apply_median_filter_3(y: np.ndarray) -> np.ndarray:
    """
    Apply a length-preserving 3-sample median filter to a 1D array.

    Edge samples are handled with edge padding so the output stays aligned to
    the input window shown in the viewer.
    """
    y = np.asarray(y)

    if y.ndim != 1:
        raise ValueError("apply_median_filter_3 expects a 1D array")
    if y.size <= 2:
        return y.astype(float, copy=False)

    y_float = y.astype(float, copy=False)
    padded = np.pad(y_float, pad_width=1, mode="edge")
    stacked = np.stack(
        [
            padded[:-2],
            padded[1:-1],
...
def compute_robust_car_median_reference(segments: np.ndarray) -> np.ndarray:
    """
    Compute a robust common-average reference using the sample-wise median across
    channels.
    """
    segments = np.asarray(segments, dtype=float)
    if segments.ndim != 2:
        raise ValueError("compute_robust_car_median_reference expects a 2D array")

    if segments.shape[0] == 0:
        return np.asarray([], dtype=float)

    return np.nanmedian(segments, axis=0)
...
def apply_filter_pipeline(
    ...
    3) Optional 3-sample median filtering.
