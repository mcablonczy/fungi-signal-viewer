...
class HDF5Viewer(QWidget):
    ...
    def _extract_ap_features_for_peak(self, y: np.ndarray, idx_peak: int, fs: float):
        """
    ...
    def _detect_peaks_in_signal(self, t: np.ndarray, y: np.ndarray):
        ...
        If peak detection is disabled, or no peaks, returns arrays of length 0.
        """
        empty = {
        ...
        for k, idx in enumerate(final_idx):
            ...
            # Baseline-relative magnitude
            # Use |y - med| for thresholds
            # Full duration at 10% of peak amplitude
    ...
    def _build_peaks_rows_for_current_window(self):
        ...
        for ch_idx in range(self.n_channels):
            ...
            for k in range(peak_idx.size):
                ...
                ap = self._extract_ap_features_for_peak(y, local_idx, self.sample_rate)
