...
# Allow running this file directly (for example via Spyder %runfile)
# while keeping absolute package imports working.
...
from plotting_gui.filters import (
    ...
    apply_filter_pipeline, compute_robust_car_median_reference,
)
...
def nice_125(value: float) -> float:
    """
    Round value to a 'nice' 1â€“2â€“5 Ã— 10^n number.
...
class HDF5Viewer(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("HDF5 Signal Viewer â€” Two-Column, L-Shape Cursor Scale")
        ...
        self.median_filter_checkbox = None
        ...
        self.peaks_use_relative = True             # default: use kÃ—Ïƒ
        self.peaks_k_prom = 4.0                    # prominence = 4Ïƒ
    ...
    def _on_median_filter_toggled(self, *args, **kwargs):
        self.median_filter_enabled = bool(
            self.median_filter_checkbox is not None
            and self.median_filter_checkbox.isChecked()
        )
    ...
    def _current_peak_settings_snapshot(self):
        ...
        cm_reference_mode = str(getattr(self, "cm_reference_mode", "single"))
        cm_ref_name = ""
        if (
            cm_enabled
            and cm_reference_mode == "single"
            and getattr(self, "cm_ref_combo", None) is not None
        ):
        ...
        return {
            ...
            "cm_reference_mode": cm_reference_mode,
    ...
    def _init_start_datetime_from_filename(self, file_path: str):
        ...
        # Simple YY->YYYY mapping: 00â€“69 -> 2000â€“2069, 70â€“99 -> 1970â€“1999
    ...
    def _on_filter_params_changed(self):
        """Sync GUI â†’ state, clear filter caches, trigger redraw."""
    ...
    def _filter_signature(self):
        return (
            bool(getattr(self, "cm_enabled", False)),
            str(getattr(self, "cm_reference_mode", "single")),
            int(self.cm_ref_index) if self.cm_ref_index is not None else None,
            bool(self.filter_enabled),
            str(self.filter_type),
            float(self.filter_f1),
            float(self.filter_f2),
            int(self.filter_order),
            bool(getattr(self, "median_filter_enabled", False)),
    ...
    def _summarize_peak_data(
        ...
        # Window duration â†’ frequency per minute
        t0 = float(t[0])
        t1 = float(t[-1])
    ...
    def _on_peaks_params_changed(self):
        """Sync GUI â†’ peak detection state and trigger redraw."""
    ...
    def load_file(self):
        ...
        self.file_label.setText("Loading fileâ€¦ please wait")
        ...
        if self.start_datetime is not None and self.win_start_dtedit is not None:
            ...
            # No real-time info â†’ disable controls
    ...
    def _build_channel_ui(self):
        ...
        self._filter_caches = {i: _LRUCache(max_segments=12) for i in range(len(self.channel_names))}
        self._cm_reference_cache = _LRUCache(max_segments=6)  # <-- NEW
    ...
    def _populate_cm_ref_combo(self):
        ...
        self._update_cm_widget_states()

        self.cm_ref_combo.blockSignals(False)

    def _update_cm_widget_states(self):
        """Enable channel selection only for single-channel referencing."""
        if self.cm_ref_combo is None:
            return

        has_channels = bool(self.channel_names)
        mode = str(getattr(self, "cm_reference_mode", "single"))
        self.cm_ref_combo.setEnabled(has_channels and mode == "single")

    def _on_cm_params_changed(self):
        """Update common-reference state from GUI and refresh."""
        if self.cm_enable_checkbox is not None:
            self.cm_enabled = self.cm_enable_checkbox.isChecked()

        if self.cm_mode_combo is not None:
            data = self.cm_mode_combo.currentData()
            self.cm_reference_mode = str(data) if data is not None else "single"

        if self.cm_ref_combo is not None and self.cm_ref_combo.count() > 0:
    ...
    def _rebuild_plots(self, preserve_xrange=None, preserve_yrange=None):
        ...
        if self.signal_source is None or not self.selected_channels:
            ...
            self.cursor_label.setText("Cursor â€” t: â€“, y: â€“, channel: â€“")
            return
    ...
    def _update_scalebar_for_plot(self, pw: pg.PlotWidget, mx: float, my: float):
        """Draw an L-shaped scalebar with 1â€“2â€“5 steps and correct units (x: s/ms, y: ÂµV/mV/V/nV)."""
        ov = self._overlays.get(pw)
        if not ov:
            return
        ...
        # ----------------- VERTICAL (AMPLITUDE, data in ÂµV) -----------------
        # IMPORTANT: we assume self.signal_source / y are in microvolts (ÂµV)
        raw_dy_uV = px_dy * bar_px  # ÂµV
        v_abs = abs(raw_dy_uV)

        if v_abs >= 1e6:
            # 1e6 ÂµV = 1 V
            y_unit = "V"
            unit_scale_y_uV = 1e6      # ÂµV per V
            val_y = v_abs / 1e6        # in V
        elif v_abs >= 1e3:
            # 1e3 ÂµV = 1 mV
            y_unit = "mV"
            unit_scale_y_uV = 1e3      # ÂµV per mV
            val_y = v_abs / 1e3        # in mV
        elif v_abs >= 1.0:
            # 1 ÂµV and up
            y_unit = "ÂµV"
            unit_scale_y_uV = 1.0      # ÂµV per ÂµV
            val_y = v_abs              # in ÂµV
        else:
            # below 1 ÂµV: show nV
            # 1 nV = 1e-3 ÂµV
            y_unit = "nV"
            unit_scale_y_uV = 1e-3     # ÂµV per nV
            val_y = v_abs / 1e-3       # in nV

        nice_val_y = nice_125(val_y)
        nice_val_y = max(1.0, nice_val_y)       # keep whole-number-ish label
        dy_uV = nice_val_y * unit_scale_y_uV    # ÂµV to draw in data units

        # Vertical bar: cursor -> up
        vx0, vy0 = mx, my
        vx1, vy1 = mx, my + dy_uV   # note: y is in ÂµV, so dy_uV is correct
    ...
    def _get_robust_car_median_reference_segment(self, sa: int, sb: int):
        """
        Return the sample-wise median across all channels for [sa:sb).

        The result is cached per range because it is shared across channels.
        """
        if self.signal_source is None or self.n_channels <= 0:
            return np.asarray([], dtype=float)

        key = (sa, sb, "robust_car_median")
        arr = self._cm_reference_cache.get(key)
        if arr is not None:
            return arr

        segments = []
        for ref_idx in range(self.n_channels):
            seg = self._get_segment(ref_idx, sa, sb)
            if seg.size == 0:
                continue
        ...
        ref = compute_robust_car_median_reference(np.vstack(segments))
        self._cm_reference_cache.put(key, ref)
        return ref

    def _get_cm_reference_segment(self, sa: int, sb: int):
        """Return the active reference trace for the current CM mode."""
        if not getattr(self, "cm_enabled", False):
            return None

        mode = str(getattr(self, "cm_reference_mode", "single"))
        if mode == "robust_car_median":
            ref = self._get_robust_car_median_reference_segment(sa, sb)
            return ref if ref.size > 0 else None

        if self.cm_ref_index is None:
            return None
        if self.cm_ref_index < 0 or self.cm_ref_index >= self.n_channels:
            return None

        return self._get_segment(self.cm_ref_index, sa, sb)
    ...
    def _get_common_mode_segment(self, ch_idx: int, sa: int, sb: int):
        ...
        ref = self._get_cm_reference_segment(sa, sb)
        if ref is None or np.asarray(ref).shape != raw.shape:
            return raw
    ...
    def _get_filtered_segment(self, ch_idx: int, sa: int, sb: int):
        ...
        use_median = bool(getattr(self, "median_filter_enabled", False))
        ...
        cm_ref = None
        if cm_enabled:
            cm_ref = self._get_cm_reference_segment(sa, sb)
            if cm_ref is None or np.asarray(cm_ref).shape != raw.shape:
                cm_enabled = False
    
        # If no filters at all, just return the raw signal (no CM, no Butterworth, no median, no MA)
        if not cm_enabled and not use_butter and not use_median and not use_ma:
            return raw
    
        # Per-channel filter cache (for CM + Butterworth + median + MA combined)
        cache = self._filter_caches.get(ch_idx)
        if cache is None:
            cache = _LRUCache(max_segments=12)
            self._filter_caches[ch_idx] = cache
    
        # Signature includes common-reference mode plus Butterworth/MA params.
        ...
        y = apply_filter_pipeline(
            ...
            median_enabled=use_median,
            ma_enabled=use_ma,
            ma_points=int(self.ma_points),
            ma_passes=int(self.ma_passes),
        )
    
        # Cache final result (CM + Butterworth + median + MA)
        cache.put(key, y)
        return y
    ...
    def _on_mouse_moved(self, pos, pw: pg.PlotWidget, ch_idx: int):
        ...
        if ctext is not None:
            ...
            if ctext is not None:
                ...
                y_str = f"{y:.2f} ÂµV"                       # assuming data are in ÂµV
                label = f"{ts_str}\n{y_str}"
            
                # Get pixel â†’ data scale
                px_dx, px_dy = vb.viewPixelSize()
